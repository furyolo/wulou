    ...
    manifest = {
        ...
        "import_contract": {
            ...
            "notes": [
                ...
                "E 列编号原样照抄回传即可：本机按编号原值等值反查，不校验前缀或格式。编号是署名制——谁建的目录谁定编号，任何人都可以用自己的“字母+数字”串，不存在全局统一格式；“ZCSQG<YYYYMMDD>SF<NN>”只是本机主人的署名写法（SF 为其署名），只在主人自己或本机 Skill 新发编号时使用。目录里现存的编号因此有多种前缀（ZCSQG / ZCSZKH / ZCSQGLQ / ZCSQGCYLZ / CSZSDCF 等），它们全都是合法编号，不要按固定格式筛选、改写或补全，也不要因为某个编号“看起来不规范”就跳过它。",
